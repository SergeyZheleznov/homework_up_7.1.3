...
public:
    ...
    Vector(const Vector&) = delete;  // Здесь мы запретили конструктор копирования
    Vector& operator=(const Vector&) = delete;  // Здесь мы запретили оператор присваивания
...
{                        
    ...
    data = new T[2];        // То есть при создании объекта класса без параметров у нас появляется массив с одним элементов. Мне так спокойнее и понятнее.   
}
